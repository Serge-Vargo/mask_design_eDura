''' Generate the mask for a 36 electrode rodent eDura '''

import functools

import ezdxf as dxf
import numpy as np
import sd_utils_v1_3 as ed
...
# ==== Workspace variables ====================================================
# File parameters
FILE_ROOT = 'gen_v0_32el\eDura_v0_ROD_TEST'
# Layers
...
# eDura outline
S_B = 5000 #(um) side of body of eDura
W_A = 2500 #(um) width of arm
L_A = 15000 #(um) length of arm
GAP_EDURA = 100    #(um) thickness of eDura outline (PDMS cut tolerance pruposes)

# electrodes
N_TRODES = 36      #() number of electrodes
TRODE_PITCH = 750  #(um) eletrode pitch
TRODE_DIAM = 10    #(um) diameter of electrode
# TRODE_DIAM_METAL = TRODE_DIAM + 10      #(um) gap between via and metal
TRODE_DIAM_METAL = 50      #(um) gap between via and metal

# traces
TRACE_PITCH = 100  #(um) trace pitch
TRACE_WIDTH = 4   #(um) trace pitch
...
PAD_TRACE = 50     #(um) trace start thickness coming out of pad
GAP_ETCH = 10      #(um) gap between via and metal
...
# N_PAD_Y = 4        #() number of bond pads in y-direction on each side
...
outline_points = ed.edura_sq_outline_points(top_right_intersection, S_B, L_A, W_A)
msp.add_lwpolyline(outline_points, 'xyseb', {'closed':'True', 'layer': EDURA_INT})
# Add gap
top_r_inter_gap = ed.arm_sq_body_intersection(S_B + GAP_EDURA, W_A + GAP_EDURA)
outline_points_gap = ed.edura_sq_outline_points(top_r_inter_gap, S_B + GAP_EDURA,
                                                L_A + GAP_EDURA/4, W_A + GAP_EDURA) # gap/4 is to adjust side edges spacing
msp.add_lwpolyline(outline_points_gap, 'xyseb', {'closed':'True', 'layer': EDURA_EXT})


''' Electrodes '''
trodes_side, points, p_x, p_y = ed.points_hexagonal_lattice_sq_grid(N_TRODES, center, TRODE_PITCH)
...
diag, hor, end_points = ed.traces_electrodes_out(points, p_x, p_y, TRACE_WIDTH,
                           TRODE_DIAM + GAP_ETCH, body_trace_spacing)
for row in diag:
...
# Run 1
hor_traces, dia_traces, end_points = ed.traces_center_to_arms_sq(end_points, N_TRODES,
                                        TRACE_PITCH, TRACE_WIDTH, p_y, p_x, body_trace_spacing)
...
''' Bond Pad traces '''
diag_r, hor_r = ed.bond_pads_traces(pts_pads_r, TRACE_PITCH, TRACE_WIDTH, N_PAD_X,
                                    N_PAD_Y, PAD_TRACE, PAD_WIDTH, PAD_HEIGHT,
                                    PAD_SPACE_X, PAD_SPACE_Y, MEET_X)
...
diag_l, hor_l = ed.bond_pads_traces(pts_pads_l, TRACE_PITCH, TRACE_WIDTH, N_PAD_X,
                                    N_PAD_Y, PAD_TRACE, PAD_WIDTH, PAD_HEIGHT,
                                    PAD_SPACE_X, PAD_SPACE_Y, MEET_X)
...
# file_name = FILE_ROOT + f"ne{N_TRODES}pch{TRODE_PITCH}w{W_A//1000}l{L_A//1000}.dxf"
