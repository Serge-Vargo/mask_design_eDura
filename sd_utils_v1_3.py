    ...
    for idx_row, row in enumerate(points):
        ...
        for idx, pt in enumerate(row):
            ...
            if idx_row < len(points)//2:     # This if-else statement is used to ease the routing of traces
                if spacing_trace == None:
                    end_point_h = [row[idx][0] + dir*((len(row)-1)-idx)*pitch_x + dir*pitch_x/2 + dir*pitch_x/8, \
                                   init_point_h[1]]
                else:
                    end_point_h = [row[idx][0] + dir*((len(row)-1)-idx)*pitch_x + dir*pitch_x/2 + dir*pitch_x/8 - dir*((len(row)-1)-idx)*spacing_x, \
                                   init_point_h[1]]
...
Organize points in quadrants to help with tracing
    ...
    --> Identify list of points in quadrant via if statements and group together
'''
def organize_in_sides(points):
    #==========================================================================
    # Quadrants are used for processing the coordinates of traces more accurately
    # The order in which points are grouped in quadrants is: Q II, Q III, Q IV, Q I
    pts_left = []  # rows in 2nd quadrant
    pts_right = [] # rows in 3rd quadrant
...
Traces from body of device towards arms (assuming symmetry):
    ...
    2nd -> Horizontal traces extending towards outside of arm
'''
def traces_center_to_arms_sq(points, n_electrodes, trace_p, trace_w, p_y, p_x, spacing_trace):

    py_f_top = (trace_p / 2) * np.ones(n_electrodes//4)
    increment_pitch_trace_y = trace_p * np.arange(n_electrodes//4)
    py_f_top = py_f_top + increment_pitch_trace_y
    py_f_top = py_f_top[::-1]
    py_f_bott = (-1)*py_f_top

    print("Final Y-coordinate of traces - TOP")
    for pt in py_f_top:
        print(pt)

    print("Final Y-coordinate of traces - BOTT")
    for pt in py_f_bott:
        print(pt)

    pts_left, rows_symm, rows_not_symm = organize_in_sides(points)
...
def bond_pads_traces(points=[0,0], trace_pitch=400, trace_w=10, n_pad_x=4, n_pad_y=4,
                     trace_pad = 20, pad_w=350, pad_h=350, pad_space_x=150,
                     pad_space_y=150, final_x=1000):
    diag_traces = []
    hor_traces = []
    ...
    for idx_row, row in enumerate(points):
        ...
        for idx, pt in enumerate(row):
            ...
            #                       end_point_d + [trace_w, trace_w] ])
            diag_traces.append([ init_point_d + [trace_pad, trace_w] , \
                                  end_point_d + [trace_pad, trace_w] ])
            #horizontal
            # init_point_h = [end_point_d[0], end_point_d[1]]
